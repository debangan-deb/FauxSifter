...
def predict():
    ...
    with pd.ExcelWriter(out,engine="xlsxwriter") as w:
        ...
        # widths so image fits left block and title sits clearly on the right
        sh.set_column('A:C',30)
        sh.set_column('D:G',32)
        sh.set_column('H:H',25); sh.set_column('I:I',20)
        [sh.set_row(r,28) for r in range(0,11)]

        # insert image FIRST (kept inside A1:C10), then write title on the right
        if img_buf:
