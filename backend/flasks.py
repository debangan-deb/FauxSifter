...
def extract_clean_title_from_dp(pg):
    sels = [
        "span#productTitle",
        "h1#title span",
        "#title span",
        "[data-feature-name='title'] span#productTitle",
    ]
    title = None
    for sel in sels:
        try:
    ...
    if not title:
        try:
            t = pg.eval_on_selector("meta[property='og:title']", "el => el.getAttribute('content')") or ""
            t = t.strip()
            if t: title = t
        except: pass
    if not title:
        try:
            t = pg.title() or ""
            t = re.sub(r"\s*\|\s*Amazon\.in\s*$", "", t).strip()
            t = re.sub(r"^Amazon\.in:\s*Buy\s+", "", t).strip()
            if t: title = t
        except: pass
    if title and re.search(r"\b\d+\s*star\b|\b\d+\s*%\b|\bpercent\b", title.lower()):
        title = None
    return title
...
def predict():
    ...
    try:
        with sync_playwright() as p:
            ...
            # 1) Reviews page: scrape reviews ONLY (do not read title here)
            safe_goto(pg, rurl(asin,1), timeout=35000, retries=1)
            pg.wait_for_selector('[data-hook="review"]', timeout=15000)

            def scrape(p):
                data = p.eval_on_selector_all('[data-hook="review"]', """
                  els=>els.map(e=>{
                    const g=s=>e.querySelector(s);const t=n=>n?(n.textContent||'').trim():'';
                    const a=s=>Array.from(e.querySelectorAll(s)).map(x=>x.textContent.trim()).join(' ').trim();
                    const rEl=g('[data-hook="review-star-rating"] .a-icon-alt')||g('.a-icon-alt')||
                      Array.from(e.querySelectorAll('[aria-label]')).find(x=>(x.getAttribute('aria-label')||'').toLowerCase().includes('out of 5 stars'));
                    const rStr=rEl?(rEl.textContent||rEl.getAttribute('aria-label')||'').trim():'';
                    const rating=(rStr.match(/([0-9]+(?:\\.[0-9]+)?)/)||['',''])[1]||'';
                    return{id:e.id||'',tt:a('[data-hook="review-title"] span'),bd:a('[data-hook="review-body"] span'),
                           nm:t(g('.a-profile-name')),dt:t(g('[data-hook="review-date"]')),rt:rating};
                  })
                """)
                for r in data:
            ...
            while c < 120:
                ...
                try:
                    pg.wait_for_function("""prev=>{
                        const ids=[...document.querySelectorAll('[data-hook="review"]')].map(e=>e.id||'');
                        if(!ids.length)return false;
                        const s=new Set(prev||[]);
                        if(ids.length!==s.size)return true;
                        return ids.some(id=>!s.has(id));
                    }""", arg=list(prev), timeout=9000)
                except: time.sleep(0.4)
                try: pg.wait_for_selector('[data-hook="review"]', timeout=7000)
                except: break
                c += 1; scrape(pg)

            # 2) DP page: get true product title + image
            safe_goto(pg, dpurl(asin), timeout=35000, retries=1)
            t = extract_clean_title_from_dp(pg)
            if t: title = t
    ...
    df = pd.DataFrame(rows, columns=["SL NO","Review Title","Review Text","Used Text","Text Source","Reviewer Name","Review Date","Rating"])
    ...
    with pd.ExcelWriter(out, engine="xlsxwriter") as w:
        ...
        # Bold, UPPERCASE, Arial 14, centered
        ...
        # layout: image A1:C10, title D1:I10 (to the right)
        sh.set_column('A:C',30)
        sh.set_column('D:I',32)
        sh.set_column('J:J',20)
        for r in range(0,11): sh.set_row(r,28)

        if img_buf:
            sh.insert_image('A1','product.png',{'image_data':img_buf,'x_scale':0.72,'y_scale':0.72,'x_offset':6,'y_offset':6})

        sh.merge_range('A1:C10','',n)
        sh.merge_range('D1:I10',(title or "UNKNOWN").upper(),h)
    ...
    for r in ws.iter_rows(min_row=1, max_row=ws.max_row, min_col=1, max_col=ws.max_column):
        for c in r:
            c.alignment = al
            # keep title block (D..I, rows 1..10) and header row bold
            c.font = Font(name='Arial', size=14, bold=(c.row==14 or (1<=c.row<=10 and 4<=c.column<=9)))
    # explicitly set D1 with uppercase title, bold, centered
    tcell = ws["D1"]; tcell.value = (title or "UNKNOWN").upper(); tcell.font = Font(name='Arial', size=14, bold=True); tcell.alignment = al
    ...
    for r, p in zip(idx, preds):
        cell = ws.cell(row=r, column=9, value="REAL" if p else "FAKE")
        cell.font = Font(name='Arial', size=14, bold=not p)
